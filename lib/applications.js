    ...
    deploy_container(application_name, container, deploy_container_callback) {
        ...
        async.waterfall([
                ...
                self.core.cluster.myriad.persistence.get([constants.myriad.APPLICATION_PREFIX, application_name].join(constants.myriad.DELIMITER), (err, application) => {
                    ...
                    try {
                        ...
                        if(!_.isEmpty(application.health_checks)) {
                            ...
                            const enforce_health_check_exists = {
                                tags: {
                                    metadata : {
                                        health_checks : {}
                                    }
                            ...
                            _.merge(container, enforce_health_check_exists);
                            // clear container health checks results from metadata
                            // in case it has data from old application health check
                            container.tags.metadata.health_checks = {};

                            _.each(application.health_checks, (health_check) => {
                                container.tags.metadata.health_checks[health_check.id] = clear_health_check;
                            });
                        } else if(container && container.tags && container.tags.metadata && container.tags.metadata.health_checks) {
                            delete container.tags.metadata.health_checks;
                        }
            ...
            (configuration, callback) => {
                delete container.health_checks;
