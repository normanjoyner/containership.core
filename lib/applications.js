...
Applications.prototype.add = function(config, fn){
    ...
    this.core.cluster.myriad.persistence.keys(constants.myriad.APPLICATIONS, function(err, applications){
        if(!_.contains(applications, [constants.myriad.APPLICATION_PREFIX, config.id].join("::"))){
            ...
            self.core.cluster.myriad.persistence.get([constants.myriad.APPLICATION_PREFIX, config.id].join(constants.myriad.DELIMITER), function(err, application){
                if(err)
                    return fn(err);
                ...
                _.merge(application, config);

                self.core.cluster.myriad.persistence.set([constants.myriad.APPLICATION_PREFIX, config.id].join(constants.myriad.DELIMITER), JSON.stringify(application), function(err){
                    if(err)
                        return fn(err);

                    var application_name = config.id;
                    delete config.id;

                    self.get_containers(application_name, function(err, containers){
                        async.each(containers, function(container, fn){
                            _.merge(container, config);
                            self.core.cluster.myriad.persistence.set([constants.myriad.CONTAINERS_PREFIX, application_name, container.id].join(constants.myriad.DELIMITER), JSON.stringify(container), function(err){
                                return fn();
                            });
                        }, fn);
                    });
...
Applications.prototype.redeploy_containers = function(application_name, fn){
    ...
    this.get_containers(application_name, function(err, containers){
        if(err)
            return fn(err);
        ...
        async.each(containers, function(container, fn){
            async.series([
                function(fn){
                    self.deploy_container(application_name, _.omit(container, ["id", "host", "start_time"]), function(){
                        return fn();
                    });
                },
                function(fn){
                    self.remove_container(application_name, container.id, function(){
                        return fn();
                    });
                }
            ], fn);
