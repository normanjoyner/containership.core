    ...
    this.myriad.listen(function(){
        ...
        self.legiond.on("node_removed", function(node){
            if(self.praetor.is_controlling_leader()){
                ...
                self.myriad.persistence.keys(constants.myriad.APPLICATIONS, function(err, applications){
                    var containers = [];

                    async.each(applications, function(application_name, fn){
                        self.myriad.persistence.get(application_name, function(err, configuration){
                            if(_.isNull(err)){
                                try{
                                    configuration = JSON.parse(configuration);
                                    var matching = _.filter(configuration.containers, function(container){
                                        return container.host == node.id;
                                    });
                        ...
                        async.each(_.flatten(containers), function(container, fn){
