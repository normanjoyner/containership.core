...
class Cluster {
    ...
    initialize(initialize_callback) {
        ...
        this.myriad.listen(() => {
            ...
            Ohai.detect((err, data) => {
                ...
                self.set_cluster_id(initialize_callback);
            });
    ...
    set_cluster_id(callback) {
        if(this.core.options.cluster_id) {
            this.core.cluster_id = this.core.options.cluster_id;

            this.myriad.persistence.set(constants.myriad.CLUSTER_ID, this.core.cluster_id, () => {
                this.core.cluster.legiond.send({
                    event: constants.events.CLUSTER_ID,
                    data: {
                        cluster_id: this.core.cluster_id
                    }
                });
            ...
            this.myriad.persistence.get(constants.myriad.CLUSTER_ID, (err, cluster_id) => {
                if(err && err.name !== 'ENOKEY') {
                    ...
                    this.core.cluster_id = crypto.generate_uuid();
