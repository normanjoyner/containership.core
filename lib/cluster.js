...
Cluster.prototype.initialize = function(fn){
    ...
    this.legiond.on("applications.sync", function(apps){
        if(self.core.options.mode == "leader"){
            ...
            self.core.applications.snapshot(function(){});
        }
    });
    ...
    this.legiond.on("container.loaded", function(container_config){
        self.core.loggers["containership.core"].log("info", [[container_config.application_name, container_config.id].join("-"), "was loaded"].join(" "));
    ...
    this.legiond.on("container.unloaded", function(container_config){
        self.core.loggers["containership.core"].log("info", [[container_config.application_name, container_config.id].join("-"), "was unloaded"].join(" "));

        if(!_.isUndefined(container_config.error)){
            self.core.loggers["containership.core"].log("verbose", error.message);
            self.core.loggers["containership.core"].log("debug", error.details);
        }
