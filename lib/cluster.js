...
class Cluster {
    constructor(core) {
        ...
        this.gatekeeper = function(data, callback) {
            if(data.metadata && data.metadata.containership && data.metadata.containership.connect_token) {
                ...
                if(valid_connect_token) {
                    ...
                    return callback(new Error(`Rejected connection attempt from ${data.id}. Node has invalid connect token`));
                }
            } else {
                return callback(new Error(`Rejected connection attempt from ${data.id}. Node has missing containership connect token metadata`));
            }
        };

        // ensure connect token validity if enabled
        if(core.options['connect-token']) {
            core.options.legiond.gatekeepers.push(this.gatekeeper);
        }
